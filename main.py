...
imageNames = ['original image', 'smoothed image', 'gradients', 'gradient directions', 'maxima', 'thresholded maxima',
              'Canny edges']
...
def findGradients(image):
    ...
    # Find the gradient direction
    gDirs = np.floor((np.arctan2((Image_gy + 0.001),
                                 (Image_gx + 0.001)) + math.pi) / math.pi * 4)  # add 0.001 to avoid dividing by zero
    return gMags, gDirs
...
def suppressNonMaxima(magnitude, gradientDirs):
    ...
    for x in range(width):  # Loop over every pixel of the image
        for y in range(height):
            ...
            try:
                if max >= magnitude[(y + offset[dir][1]), (x + offset[dir][0])] and max >= magnitude[
                    (y - offset[dir][1]), (x - offset[dir][0])]:
                    maximaImage[y, x] = magnitude[y, x]  # if it is a local maxima, set the pixel value
...
def mouse(button, state, x, y):
    ...
    if state == GLUT_DOWN:
        ...
        if button == GLUT_LEFT_BUTTON and initX == x and initY == y:  # Process a left click (with no dragging)
            ...
            print
            'click at', pixelX, pixelY, '=', image[pixelY][pixelX]
